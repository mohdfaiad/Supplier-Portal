    ...
    public class RepAssembleOrderOperator : ReportBase
    {
        ...
        public override bool FillValues(String templateFileFolder, String templateFileName, IList<object> list)
        {
            try
            {
                ...
                //生产线编号	
                this.SetRowCell(pageIndex, 2, 1, orderMaster.Flow + "[" + orderMaster.FlowDescription + "]");

                //物料代码+描述
                this.SetRowCell(pageIndex, 3, 0, orderDetails[0].Item + "[" + orderDetails[0].ItemDescription + "]");

                //Van号
                barCode = Utility.BarcodeHelper.GetBarcodeStr(orderMaster.TraceCode, this.barCodeFontName);
                this.SetRowCell(pageIndex, 4, 1, barCode);

                this.SetRowCell(pageIndex, 6, 0, orderMaster.TraceCode);
