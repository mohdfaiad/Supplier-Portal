...
{
    public class RepBatchProductionOrderOperator : ReportBase
    {
        ...
        public override bool FillValues(string templateFileFolder, String templateFileName, IList<object> list)
        {
            try
            {
                ...
                foreach (IList<object> listdet in list)
                {
                    pageIndex++;
                    orderIndex = 1;
                    this.init(templateFileFolder.Replace("\r\n\t\t", "") + templateFileName, ROW_COUNT);
                    if (listdet == null || listdet.Count < 3) return false;

                    PrintOrderMaster orderMaster = (PrintOrderMaster)(listdet[0]);
                    IList<PrintOrderDetail> orderDetails = (IList<PrintOrderDetail>)(listdet[1]);
                    IList<PrintOrderBomDetail> orderBomDetails = (IList<PrintOrderBomDetail>)(listdet[2]);
