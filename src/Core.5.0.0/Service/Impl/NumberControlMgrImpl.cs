...
namespace com.Sconit.Service.Impl
{
    public class NumberControlMgrImpl : BaseMgr, INumberControlMgr
    {
        ...
        public IDictionary<string, decimal> GetHuId(OrderDetail orderDetail)
        {
            ...
            //parm[0] = new SqlParameter("@LotNo", SqlDbType.VarChar, 50);
            //parm[0].Value = orderDetail.LotNo;

            //parm[1] = new SqlParameter("@Item", SqlDbType.VarChar, 50);
            //parm[1].Value = orderDetail.Item;

            //parm[2] = new SqlParameter("@Qty", SqlDbType.Decimal);
            //parm[2].Precision = 18;
            //parm[2].Scale = 8;
            //parm[2].Value = orderDetail.HuQty;

            //parm[3] = new SqlParameter("@UC", SqlDbType.Decimal);
            //parm[3].Precision = 18;
            //parm[3].Scale = 8;
            //parm[3].Value = orderDetail.UnitCount;

            //parm[4] = new SqlParameter("@ManufactureParty", SqlDbType.VarChar, 50);
            //parm[4].Value = orderDetail.ManufactureParty;
            ...
            var HuIds = GetHuId(orderDetail.LotNo, orderDetail.Item, orderDetail.ManufactureParty,
                orderDetail.HuQty, orderDetail.UnitCount);
        ...
        {
            var HuIds = GetHuId(item.LotNo, item.Code, item.ManufactureParty,
              item.HuQty, item.HuUnitCount);
        ...
        public IDictionary<string, decimal> GetHuId(string lotNo, string item, string manufactureParty, decimal qty, decimal unitCount)
        {
            ...
            #region 异步调用WMS
            var huIds = HuIds.Select(h => h.Key).ToList();
            //AsyncRecourdMessageQueue(MethodNameType.CreateHu, huIds);
            #endregion
        ...
        {
            string numberSuffix = GetNextSequence(BusinessConstants.NUMBERCONTROL_BILLOFLADINGNO);
