...
BEGIN 
	...
	BEGIN 
		...
		END			
		IF(CHARINDEX('ReceiptMasterStatusDescription',@SortCloumn)>0) 
		BEGIN 
			SET @SortCloumn=REPLACE(@SortCloumn,'ReceiptMasterStatusDescription','Status') 
		END
		IF(CHARINDEX('ExternalReceiptNo',@SortCloumn)>0) 
		BEGIN 
			SET @SortCloumn=REPLACE(@SortCloumn,'ExternalReceiptNo','ExtRecNo') 
		END
		...
		END		
		IF(CHARINDEX('Receipt',@SortCloumn)>0) 
		BEGIN 
			SET @SortCloumn=REPLACE(@SortCloumn,'Receipt','Rec') 
		END		
		IF(CHARINDEX('Receive',@SortCloumn)>0) 
		BEGIN 
			SET @SortCloumn=REPLACE(@SortCloumn,'Receive','Rec') 
		END		
		IF(CHARINDEX('CreateHuOption',@SortCloumn)>0) 
		BEGIN 
			SET @SortCloumn=REPLACE(@SortCloumn,'CreateHuOption','CreateHuOpt') 
		END		
		IF(CHARINDEX('Authority',@SortCloumn)>0) 
		BEGIN 
			SET @SortCloumn=REPLACE(@SortCloumn,'Authority','Auth') 
		END					 
