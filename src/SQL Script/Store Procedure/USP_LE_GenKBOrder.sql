...
IF EXISTS(SELECT * FROM SYS.OBJECTS WHERE TYPE='P' AND name='USP_LE_GenKBOrder')
	DROP PROCEDURE USP_LE_GenKBOrder	
GO
...
BEGIN 
	...
	CREATE TABLE #temp_LOG_GenKBOrder(
	...
	begin try
		...
		-------------------↓计算紧急需求-----------------------
		--记录日志
		set @Msg = N'计算路线需求生成KB紧急要货单开始，需求时间小于' + convert(varchar, @ReqTimeFrom, 120)
		insert into #temp_LOG_GenKBOrder(Flow, Lvl, Msg) values(@Flow, 0, @Msg)
		
		truncate table #tempOrderDet
		insert into #tempOrderDet(UUID, Flow, FlowDetId, Item, Uom, UC, MinUC, UCDesc, Container, ContainerDesc,
								ManufactureParty, LocFrom, LocTo, Routing,
								ReqQty, SafeStock, MaxStock, MinLotSize, RoundUpOpt)
		select NEWID(), det.Flow, det.FlowDetId, det.Item, det.Uom, det.UC, det.MinUC, det.UCDesc, det.Container, det.ContainerDesc, 
		det.ManufactureParty, det.LocFrom, det.LocTo, det.Routing,
		--最大库存量 - 库存 - 待收
		det.MaxStock - ISNULL(loc.Qty, 0) - ISNULL(op.RctQty, 0) as ReqQty,
		...
		left join (select op.Item, op.Location, SUM(op.OrderQty - op.FinishQty) as RctQty 
					...
					where det.Flow = @Flow and det.Strategy = 2 and op.IRType = 1 and op.ReqTime < @ReqTimeFrom
					group by op.Item, op.Location
					) as op on det.Item = op.Item and det.LocTo = op.Location  --待收
		where det.Flow = @Flow and det.Strategy = 2 and det.SafeStock > (ISNULL(loc.Qty, 0) + ISNULL(op.RctQty, 0))
		...
		begin
			...
			begin
				--获取订单号
				exec USP_GetDocNo_ORD @Flow, 2, @FlowType, 0, 0, 1, @FlowPartyFrom, @FlowPartyTo, @FlowLocTo, @FlowLocFrom, @FlowDock, 0, @OrderNo output
				...
				--记录日志
				set @Msg = N'创建KB要货单' + @OrderNo
				insert into #temp_LOG_GenKBOrder(Flow, OrderNo, Lvl, Msg) values(@Flow, @OrderNo, 0, @Msg)
			...
			--where det.OrderQty >= det.MinLotSize
		...
		begin
			--记录日志
			set @Msg = N'计算路线需求生成KB要货单开始，发单时间' + convert(varchar, @StartTime, 120)+ N'，需求时间范围' + convert(varchar, @ReqTimeFrom, 120) + ' ~ ' + convert(varchar, @ReqTimeTo, 120)
			insert into #temp_LOG_GenKBOrder(Flow, Lvl, Msg) values(@Flow, 0, @Msg)
			...
			left join (select op.Item, op.Location, SUM(op.OrderQty - op.FinishQty) as RctQty 
						from LE_FlowDetSnapShot as det
						inner join LE_OrderPlanSnapshot as op on det.Item = op.Item and det.LocTo = op.Location
						where det.Flow = @Flow and det.Strategy = 2 and op.IRType = 1 and op.ReqTime < @ReqTimeTo
						group by op.Item, op.Location
						) as op on det.Item = op.Item and det.LocTo = op.Location  --待收
			where det.Flow = @Flow and det.Strategy = 2 and det.SafeStock <= (ISNULL(loc.Qty, 0) + ISNULL(op.RctQty, 0))
			...
			begin
				...
				begin
					--获取订单号
					exec USP_GetDocNo_ORD @Flow, 2, @FlowType, 0, 0, 0, @FlowPartyFrom, @FlowPartyTo, @FlowLocTo, @FlowLocFrom, @FlowDock, 0, @OrderNo output
	...
	begin catch
		...
		--记录日志
		set @Msg = N'计算路线需求生成KB要货单出现异常，异常信息：' +  + Error_Message()
		insert into #temp_LOG_GenKBOrder(Flow, Lvl, Msg) values(@Flow, 2, @Msg)
	end catch
	
	insert into LOG_GenKBOrder(Flow, OrderNo, Lvl, Msg, CreateDate, BatchNo) 
	select Flow, OrderNo, Lvl, Msg, CreateDate, @BatchNo from #temp_LOG_GenKBOrder
	
	drop table #temp_LOG_GenKBOrder
