...
namespace com.Sconit.Web.Controllers
{
    public class ReportBaseController : WebAppBaseController
    {
        ...
        public IList<Location> GetReportLocations(string sapLocation, string plantFrom, string plantTo, string regionFrom, string regionTo, string locationFrom, string locationTo)
        {
            ...
            {
                locationList = base.genericMgr.FindAll<Location>("from Location l where l.SAPLocation = ? ", new object[] { sapLocation });
                ViewBag.Location = Resources.View.LocationDetailView.LocationDetailView_SAPLocation;
            }
            else if (!string.IsNullOrEmpty(plantFrom))
            {
                if (!string.IsNullOrEmpty(plantTo))
                {
                    regionList = base.genericMgr.FindAll<Region>("from Region r where r.Workshop between ? and ?", new object[] { plantFrom, plantTo });
                }
                else
                {
                    regionList = base.genericMgr.FindAll<Region>("from Region r where r.Workshop = ? ", new object[] { plantFrom });
                }
                ViewBag.Location = Resources.View.LocationDetailView.LocationDetailView_plant;
            }
            else if (!string.IsNullOrEmpty(regionFrom))
            {
                if (!string.IsNullOrEmpty(regionTo))
                {
                    regionList = base.genericMgr.FindAll<Region>("from Region r where r.Code between ? and ?", new object[] { regionFrom, regionTo });
                }
                else
                {
                    regionList = base.genericMgr.FindAll<Region>("from Region r where r.Code = ? ", new object[] { regionFrom });
                }
            ...
            else if (!string.IsNullOrEmpty(locationFrom))
            {
                ...
                {
                    locationList = base.genericMgr.FindAll<Location>("from Location l where l.Code between ? and ?", new object[] { locationFrom, locationTo });
                }
                else
                {
                    locationList = base.genericMgr.FindAll<Location>("from Location l where l.Code = ?", new object[] { locationFrom });
                }
            ...
            if (regionList.Count > 0 && locationList.Count == 0)
            {
                ...
                hql += ")";
                locationList = base.genericMgr.FindAll<Location>(hql, paras.ToArray());
            }
        ...
        public IList<Item> GetReportItems(string itemFrom, string itemTo)
        {
            ...
            if (!string.IsNullOrEmpty(itemFrom))
            {
                ...
                {
                    itemList = base.genericMgr.FindAll<Item>("from Item i where i.Code between ? and ?", new object[] { itemFrom, itemTo });
                }
                else
                {
                    itemList = base.genericMgr.FindAll<Item>("from Item i where i.Code = ? ", new object[] { itemFrom });
                }
