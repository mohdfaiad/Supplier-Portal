...
namespace com.Sconit.Web.Controllers
{
    ...
    public class WebAppBaseController : BaseController
    {
        ...
        #region 检查查询条件是否为空
        public IList<T> GetReflectionResult<T>(DataTable dataTable)
        {
            DataColumnCollection dtcolColl = dataTable.Columns;
            DataRowCollection dtrowColl = dataTable.Rows;
            ...
            for (int i = 0; i < dtrowColl.Count; i++)
            {
                T t = (T)Activator.CreateInstance(type);
                DataRow dataRow = dtrowColl[i];
                for (int coll = 0; coll < dataTable.Columns.Count; coll++)
                {
                    string colName = dtcolColl[i].ColumnName;
                    Object getValue = dataRow[colName];
                    var pInfo = properties.First(p => p.Name == colName);
                    if (!Convert.IsDBNull(getValue) && getValue.ToString() != null)
                    {
                        pInfo.SetValue(t, getValue, null);
                    }
                    else
                    {
