...
namespace com.Sconit.Web.Controllers.ACC
{
    ...
    public class RoleController : AuthorizationBaseController
    {
        ...
        /// </summary>
        private static string selectPermissionGroupByRoleId = "select rp.PermissionGroup from RolePermissionGroup as rp where rp.Role.Id = ?";

        /// <summary>
        ///  select PermissionGroups Not In Role
        /// </summary>
        private static string selectPermissionGroupsNotInRole = @"select p from PermissionGroup as p where p.Id not in (select  rp.PermissionGroup.Id from RolePermissionGroup as rp where rp.Role.Id = ?)";

        /// <summary>
        /// select Users By RoleId
        /// </summary>
        private static string selectUsersByRoleId = "select ur.User from UserRole as ur where ur.Role.Id = ? order by ur.User.Code";

        /// <summary>
        /// select Users  Not In  Role
        /// </summary>
        private static string selectUsersNotInRole = @"select u from User as u where u.Id not in (select ur.User.Id from UserRole as ur where ur.Role.Id = ?) order by u.Code";
        ...
        public ActionResult New(Role role)
        {
            ...
            {
                ////判断描述不能重复
                if (base.genericMgr.FindAll<long>(roleDuiplicateVerifyStatement, new object[] { role.Code })[0] > 0)
                {
                    SaveErrorMessage(Resources.ACC.Role.Errors_Existing_Role, role.Code);
                }
                else
                {
                    base.genericMgr.Create(role);
        ...
        public ActionResult _Edit(int? id)
        {
            ...
            {
                Role role = base.genericMgr.FindById<Role>(id);
        ...
        public ActionResult _RolePermissionGroups(int? id)
        {
            ...
            {
                ViewBag.AssignedPermissionGroup = base.genericMgr.FindAll<PermissionGroup>(selectPermissionGroupByRoleId, id.Value);
                ViewBag.UnAssignedPermissionGroup = base.genericMgr.FindAll<PermissionGroup>(selectPermissionGroupsNotInRole, id.Value);
                return PartialView();
            }
        }
        ...
        public ActionResult _RoleUsers(int? id)
        {
            ...
            {
                ViewBag.AssignedUsers = base.genericMgr.FindAll<User>(selectUsersByRoleId, id.Value);
                ViewBag.UnAssignedUsers = base.genericMgr.FindAll<User>(selectUsersNotInRole, id.Value);
                return PartialView();
            }
        }
        ...
        [HttpPost]
        [SconitAuthorize(Permissions = "Url_Role_Edit")]
        public ActionResult _RolePermissions(int? id, string permissionCategoryCode,IList<int> assignedPermissionIds)
        {
            if (!id.HasValue)
            {
                return HttpNotFound();
        ...
        public ActionResult Delete(int? id)
        {
            ...
            {
                base.genericMgr.DeleteById<Role>(id);
        ...
        public ActionResult _AjaxLoadingPermissions(int roleId, string permissionCategoryCode)
        {
            ...
            {
                Data = new SelectList[] { new SelectList(base.genericMgr.FindAll<Permission>(selectPermissionsNotInRoleByCategory, new object[] { permissionCategoryCode, roleId, permissionCategoryCode }), "Id", "LongDescription1"), new SelectList(base.genericMgr.FindAll<Permission>(selectPermissionsByRoleIdAndCategory, new object[] { permissionCategoryCode, roleId }), "Id", "Description") }
            };
        }
        ...
        private void Prepare4AssignUserPermissions(int roleId)
        {
            ...
            {
                ViewBag.AssignedPermissions = base.genericMgr.FindAll<Permission>(selectPermissionsByRoleIdAndCategory, new object[] { permissionCategory.Code, roleId });
                ViewBag.UnAssignedPermissions = base.genericMgr.FindAll<Permission>(selectPermissionsNotInRoleByCategory, new object[] { permissionCategory.Code, roleId, permissionCategory.Code });
            }
        }
    }
}
